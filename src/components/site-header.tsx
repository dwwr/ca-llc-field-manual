...
export function SiteHeader({
  ...
  site: Pick<
    SiteCopy,
    "headerName" | "headerTagline" | "menuAria" | "openMenu" | "mobileNavAria"
  >;
}) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/80 bg-background/90 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6">
        ...
        <nav className="hidden items-center gap-0.5 lg:flex" aria-label={site.menuAria}>
          {nav.map((item) => {
            const active =
              item.href === "/"
                ? pathname === "/"
        ...
        <Sheet>
          ...
          <SheetContent side="right" className="w-80">
            ...
            <nav className="flex flex-col gap-1 px-2 pb-6" aria-label={site.mobileNavAria}>
              {nav.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
